...
(define (same-parity . args)
  (define (same-parity-iter target-modulo args result)
    (cond ((null? args) result)
          ((= target-modulo (remainder (car args) 2)) (same-parity-iter target-modulo (cdr args) (cons (car args) result)))
          (else (same-parity-iter target-modulo (cdr args) result))))
  (reverse (same-parity-iter (remainder (car args) 2) args (list))))
